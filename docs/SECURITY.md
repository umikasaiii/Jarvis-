...
| Threat | Mitigation |
|--------|-----------|
| Malware / other apps reading secrets | Secrets only in Android Keystore-backed store; `allowBackup=false`; secrets excluded from backup/device-transfer rules; never in `BuildConfig`, logs, or the repo. Google Drive's local connection state (authorized flag, connected email, backup folder id — no client id/secret/token, see below) and the backup content key live in dedicated `EncryptedSharedPreferences` (`DriveCredentialStore`, `BackupKeyManager`) — separate from, and never mixed into, the plaintext DataStore every other setting uses. |
...
| Unauthorized network egress | Offline path never touches the network. Remote calls happen only under an opt-in `PrivacyProfile`, or the explicitly sanctioned exceptions (weather; optional Google Drive backup sync; later live-traffic ETA — see `docs/PRIVACY.md`), each off by default. Router records exactly what would be shared before sending. |
| Google Drive OAuth token/account compromise | `drive.file` scope only — JARVIS can only read/write files/folders it created itself (a "JARVIS Backups" folder), never the user's other Drive files. Identity/AuthorizationClient (`play-services-auth`) manages tokens; the app never stores an access or refresh token itself — `GoogleAuthManager.currentAccessToken()` asks Play Services fresh each time, silently once consent is granted. Disconnecting in JARVIS always clears local state; server-side revocation is the user's own action at myaccount.google.com/permissions (docs/GOOGLE_DRIVE_SETUP.md) rather than a guessed-at API call. A revoked/lost connection is detected and reported honestly (account shown as disconnected) rather than silently retried forever. See ADR 0015 for why this is a deliberate, narrow exception to this app's general Play-Services-free stance. Debug and release builds are separate OAuth Android clients (different package name + certificate, docs/GOOGLE_DRIVE_SETUP.md), each connected independently — the debug key is a fixed, committed, non-secret AOSP convention key (docs/DEBUG_SIGNING.md), not a substitute for release signing. |
...
| `INTERNET`, `ACCESS_NETWORK_STATE` | Opt-in PC/HA; opt-in weather (Open-Meteo, no API key); opt-in Google Drive backup sync | Unused on the offline path; weather sends only a rounded coordinate pair, degrades to "unknown" with no connection; Drive sync uploads only the already-encrypted backup archive, and only once an account is explicitly connected. |
