...
| Threat | Mitigation |
|--------|-----------|
| Malware / other apps reading secrets | Secrets only in Android Keystore-backed store; `allowBackup=false`; secrets excluded from backup/device-transfer rules; never in `BuildConfig`, logs, or the repo. Google Drive OAuth client id/secret, tokens and the backup content key live in dedicated `EncryptedSharedPreferences` (`DriveCredentialStore`, `BackupKeyManager`) — separate from, and never mixed into, the plaintext DataStore every other setting uses. |
...
| Google Drive OAuth token/account compromise | `drive.appdata` scope only — JARVIS can never read/write the user's own visible Drive files, only its own hidden per-app folder. No Google SDK/Play Services in the flow (RFC 8252 browser redirect + PKCE, not a WebView). Tokens live in `EncryptedSharedPreferences`; disconnecting revokes the token with Google and clears it locally. An expired/revoked refresh token is detected and reported honestly (account shown as disconnected) rather than silently retried forever. |
