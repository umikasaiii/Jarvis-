...
| 5 | Obsidian memory (SAF, retrieval) | **First slice implemented (pending device check)** — `VaultRepository` (SAF `OpenDocumentTree`, read-only, persisted permission) reads `.md`; `MemoryIndex` parses (core `MarkdownParser`), chunks by heading, ranks (core `RetrievalRanker`) in memory; retrieval injected as grounding context in `SessionCoordinator.generateAnswer`. Memory screen picks/reindexes the vault. Room/FTS store still pending (optimization; in-memory index is fine for a personal vault). |
