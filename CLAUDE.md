...
| 6c | Understanding V2 | **Implemented (pending CI/device check)** — `TurnPlanner` decomposes questions and actions joined by punctuation, commas or conjunctions without splitting comparisons. The fast model returns `intent|confidence`; low-confidence output cannot execute a tool and escalates to the advanced brain. Every clause receives an answer and later clauses inherit only the bounded same-turn context. |
| 6d | Persistent response queue | **Implemented (pending CI/device check)** — typed requests are persisted in Room and run by a long-running WorkManager worker with visible progress, cancellation, retry and idempotent chat writes. Model load, memory retrieval and generation survive Activity closure/process recreation. A private “response ready” notification opens the chat; preview is opt-in. |
...
- **Not implemented yet:** VAD tuning, Room/FTS vault index, HA, PC server,
